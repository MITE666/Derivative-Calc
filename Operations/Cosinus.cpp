...
std::shared_ptr<Node> Cosinus::Differentiate(std::string &var) {
    ...
    new_exp->right.lock()->right = std::weak_ptr<Node>(std::make_shared<StringNode>("sin"));
    new_exp->right.lock()->right.lock()->right = exp->right;
    return new_exp;
}
