#include "Headers/Arcsinus.h"

Arcsinus::Arcsinus(std::shared_ptr<Node>& exp_) : Expression(exp_) {}

void Arcsinus::Differentiate(std::string &var) {
    auto arg = DeepCopy(exp->right);
    auto arg_exp = ExpressionType(arg);
    arg_exp->Differentiate(var);
    std::shared_ptr<Node> new_exp = std::make_shared<StringNode>("/");
    new_exp->left = arg;
    new_exp->right = std::make_shared<StringNode>("^");
    new_exp->right->left = std::make_shared<StringNode>("-");
    new_exp->right->right = std::make_shared<FloatNode>(0.5);
    new_exp->right->left->left = std::make_shared<IntNode>(1);
    new_exp->right->left->right = std::make_shared<StringNode>("^");
    new_exp->right->left->right->left = exp->right;
    new_exp->right->left->right->right = std::make_shared<IntNode>(2);
    exp = new_exp;
}
