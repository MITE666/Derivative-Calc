...
void Arcsinus::Differentiate(std::string &var) {
    ...
    new_exp->get_right()->set_right(std::make_shared<Node<float>>(0.5));
    new_exp->get_right()->get_left()->set_left(std::make_shared<Node<int>>(1));
    new_exp->get_right()->get_left()->set_right(std::make_shared<Node<std::string>>("^"));
    new_exp->get_right()->get_left()->get_right()->set_left(arg);
    new_exp->get_right()->get_left()->get_right()->set_right(std::make_shared<Node<int>>(2));
    exp = new_exp;
}