...
std::shared_ptr<Node> Arccotangent::Differentiate(std::string &var) {
    ...
    new_exp->right.lock()->right.lock()->right = std::weak_ptr<Node>(std::make_shared<IntNode>(1));
    new_exp->right.lock()->right.lock()->left.lock()->left = exp->right;
    new_exp->right.lock()->right.lock()->left.lock()->right = std::weak_ptr<Node>(std::make_shared<IntNode>(2));
    return new_exp;
}
